				...
				<tbody>
					...
					@forelse($transactions as $transaction)
						<tr>
							...
							<th title="{{$transaction->description}}"><i class="fa fa-eye"></i></th>
							<td>{{ \Carbon\Carbon::createFromTimeStamp(strtotime($transaction->created_at), "UTC")->tz(auth()->user()->timezone)->format('d M, Y h:i:s A') }}</td>
 							<td>
 								<a href="{{ url('/deposits/' . Hashids::encode($transaction->deposit_id )) }}" 
								title="Deposite" class="btn btn-xs btn-primary">
